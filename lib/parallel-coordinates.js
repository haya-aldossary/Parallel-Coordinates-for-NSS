...
(function ($, undefined) {
	...
	/// <summary>Get mouse position relative to container.</summary>
    /// <param name="el" type="Number">The current coordinates of the element</param>
    /// <returns type="Number">X and Y positions</returns>
		...
		update: function () {
				...
				if ($("#chkDensity").attr("checked")) {
					$.each(dots, function (key, val) {
						if (val != undefined) {
							...
							c = c > 255 ? 255 : c;
							//  console.log("val="+val);
							//self.axes[col].ctx.fillStyle = 'rgba(' + c + ',' + 0 + ',' + (255 - c) + ',0.8)';
						    //self.axes[col].ctx.fillRect(19.5 - w / 2, key - 46.5, w, 9);
							var c = $('#__processing0')[0].getContext('2d');
							if (w  > 70) {
							    w = 69;
							}
						    var p = c.getImageData(w * 2  , 1, 1, 1).data;
			...
			_(filtered).each(function (d, k) {
				...
				*/
				...
				if (d.id == slicky.select_Id) { // selected row
